...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $isi = trim($_POST['isi'] ?? '');
    $gambar = handle_upload('gambar', __DIR__ . '/../uploads/berita/', $_POST['old_gambar'] ?? null);
    if ($judul === '') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($koneksi, "UPDATE berita SET judul = ?, kategori = ?, tanggal = ?, penulis = ?, status = ?, gambar = ?, ringkasan = ?, isi = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "ssssssssi", $judul, $kategori, $tanggal, $penulis, $status, $gambar, $ringkasan, $isi, $id);
    ...
    $result = mysqli_query($koneksi, "SELECT * FROM berita WHERE id = " . $id . " LIMIT 1");
