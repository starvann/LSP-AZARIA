<?php
/**
 * admin/guru/hapus.php
 * Memproses penghapusan data Data Guru & Tendik dari database.
...
if ($id > 0) {
    ...
    $result = mysqli_query($koneksi, "SELECT foto FROM guru WHERE id = " . $id . " LIMIT 1");
    $row = mysqli_fetch_assoc($result);
    if ($row && !empty($row['foto'])) {
        $filePath = __DIR__ . '/../uploads/guru/' . $row['foto'];
