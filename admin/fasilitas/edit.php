...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $foto = handle_upload('foto', __DIR__ . '/../uploads/fasilitas/', $_POST['old_foto'] ?? null);
    if ($nama_fasilitas === '') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($koneksi, "UPDATE fasilitas SET nama_fasilitas = ?, kategori = ?, kondisi = ?, kapasitas = ?, foto = ?, deskripsi = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "ssssssi", $nama_fasilitas, $kategori, $kondisi, $kapasitas, $foto, $deskripsi, $id);
    ...
    $result = mysqli_query($koneksi, "SELECT * FROM fasilitas WHERE id = " . $id . " LIMIT 1");
...
$breadcrumb = ['Data Sekolah', 'Fasilitas', 'Edit'];
